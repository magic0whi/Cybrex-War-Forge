...
1. 最小平方法: 如果有n个样本点 \\((x_1, y_1), (x_2), (y_2), \dots, (x_n, y_n)\\) , 若要求一条最能拟合这些点的直线 \\(\hat{y} = bx + a\\), 可通过下面的表达式:
...
2. 排列数: \\(A_n^m = n(n-1)(n-2)\dots(n-m+1) = \frac{n!}{(n-m)!} \qquad (m \leqslant n, m,n \in N^+)\\)
   将n个不同元素全部取出的一个排列, 记作\\(A_n^n = n! \quad (规定 0! = 1)\\)

3. 组合: 从n个不同的元素中取出 \\(m \quad (m \leqslant n)\\) 个元素合成一组 (与顺序无关, 只取不排)
4. 组合数:
   \\(C_n^m = \frac{A_n^m}{A_m^m} = \frac{n(n-1)(n-2)\dots(n-m+1)}{m!} = \frac{n!}{m!(n-m)!} \qquad (m \leqslant n, m,n \in N^+)\\)
   \\(组合式性质公式 \begin{cases} C_n^m = C_n^{n-m} \\\ C_{n+1}^m = C_n^m + C_n^{m-1} \end{cases}\\)
#### 二项式定理

1. 二项式定理: \\((a+b)^n = C_n^0 a^n + C_n^1 a^{n-1}b + C_n^2 a^{n-2}b^2 + \dots + C_n^nb^n\\)
2. 二项式系数的性质 (结合"杨辉三角"研究推出)
   二项式系数: \\(C_n^k (k \in 1, 2, 3, \dots, n) \qquad (共有 n+1 项)\\)
...
\\(则 P(x=r) = \frac{C_M^k C_{N-M}^{n-k}}{C_N^n} \qquad (r = 1, 2, 3, \dots, n)\\)
不合格数量X的概率分布表如下表所示:
| X | 0 | 1 | 2 | ... | \\(l\\) |
| - | - | - | - | - | - |
...
\\(P(X=k)=C_n^k p^k q^{n-k} \qquad (k=0, 1, 2, \dots, n)\\)
...
期望: \\(\mu = E(X) = x_1p_1 + x_2p_2 + \dots + x_np_n\\) 为随机变量的均值或数学期望
期望的性质:
1. \\(E(aX + b) = aE(X+b)\\)
...
离散方差: \\(\sigma^2 = V(X) = (x_1-\mu)^2p_1 + (x_2-\mu)^2p_2 + \dots + (x_n-\mu)^2p_n\\)
离散方差的性质:
1. \\(V(aX+b)=a^2V(X)\\)
2. \\(V(X)=E(X^2)-[E(X)]^2\\)
3. 两点分布方差\\(V(X)=p(1-p)\\)
